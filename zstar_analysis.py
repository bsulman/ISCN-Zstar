...
def plot_profile(layerdata,profiledata,profid=None,fig_text=True,clearaxis=True,*args,**kwargs):
    if clearaxis:
        cla()
    if profid is None:
        while True:
...
zstardata=pandas.read_csv('../Zstar_params_output/piecewise_%s_combined.csv'%Cmeasure,index_col='profile_name',low_memory=False)
zstardata_percent=pandas.read_csv('../Zstar_params_output/piecewise_%s_combined.csv'%'pctC',index_col='profile_name',low_memory=False)
...
figure('Profiles map US',figsize=(11.8,6.5));clf()
ax=subplot(321,projection=cartopy.crs.PlateCarree())
...
legend(ncol=1,loc=(-.3,-.2),markerscale=3,fontsize='small')
ax.set_title('Soil orders')
xlim(-129,-65)
ylim(23,52)
labx=0.02
...
cb=colorbar(h,extend='max');cb.set_label('$C_{surf}$ (%s)'%Cunits)
xlim(-129,-65)
ylim(23,52)
...
ax.set_title('$C_{deep}$')
cb=colorbar(h,extend='max');cb.set_label('$C_{deep}$ (%s)'%Cunits)
xlim(-129,-65)
ylim(23,52)
text(labx,laby,'(f)',transform=gca().transAxes)
...
if Cmeasure=='Cstock':
    ...
    ax.set_title('SOC stocks to %d cm'%Zmax)
    cb=colorbar(h,extend='max');cb.set_label('SOC stock (%s)'%'kgC m$^{-2}$')
    xlim(-129,-65)
    ylim(23,52)
...
ax.set_title('$Z_{min}$')
cb=colorbar(h,extend='max');cb.set_label('$Z_{min}$ (cm)')
xlim(-129,-65)
ylim(23,52)
...
figure('Ap effect maps',figsize=(9,9));clf()
...
figure('Ap horizon histograms',figsize=(8,8));clf()
labx=0.02
laby=0.93
...
if Cmeasure=='Cstock':
    ...
    # legend()
    title('SOC stocks to %d cm'%Zmax)
    xlabel('SOC stock values (%s)'%'kgC m$^{-2}$')
    ylabel('Distribution')
    text(labx,laby,'(e)',transform=gca().transAxes)
...
figure('Ap direct comparison',figsize=(6.8,6.9));clf()
...
figure('Ap direct comparison histograms',figsize=(5.5,7.5));clf()
...
ax.set_title('SOC stocks to %d cm (Ap minus no-Ap)'%Zmax)
cb=colorbar(h,extend='both');cb.set_label('SOC stock (%s)'%'kg C m$^{-2}$')
xlim(-129,-65)
ylim(23,52)
text(0.03,1.03,'(a)',transform=ax.transAxes)
...
ax.set_title('SOC stocks to %d cm (%% difference Ap minus no-Ap)'%Zmax)
cb=colorbar(h,extend='both');cb.set_label('% difference')
xlim(-129,-65)
ylim(23,52)
text(0.03,1.03,'(b)',transform=ax.transAxes)
...
xlabel('SOC stock to %d cm (%s)'%(Zmax,'kg C m$^{-2}$'))
ylabel('Number of cells')
text(-Cstocks_vmax*0.7,100,'Less SOC with Ap',ha='center')
text(Cstocks_vmax*0.7,100,'More SOC with Ap',ha='center')
...
ylabel('Number of cells')
text(-75,75,'Less SOC with Ap',ha='center')
text(75,75,'More SOC with Ap',ha='center')
...
xlabel('SOC stock without Ap (kg C m$^{-2}$)')
ylabel('SOC stock with Ap (kg C m$^{-2}$)')
title('SOC stock differences')
ylim(-1,31)
xlim(-1,31)
...
f=figure('Land use bar plot');clf()
...
figure('Ap effect by land use and soil order');clf()
forest_delim=3.75
subplot(231)
...
text(forest_delim+0.25,ylims[1]*0.99,'Forest',fontsize='small',ha='left',va='top')
ylim(ylims)
ylabel('SOC stocks to 30 cm (kgC m$^{-2}$)')
title('SOC stock')
...
SOC stock to 30 cm = {Cstock:1.1f} kgC m$^{{-2}}$''').format(
                                        profile=profid,
                                        # profiledata.landuse[profid],
                                        order=get_soil_order(profiledata.soil_taxon[profid]),
                                        zstar=profiledata.zstar[profid],
...
fig,axes=subplots(2,2,num='Profile examples',squeeze=False)
sca(axes[0,0])
...
def save_all_figs(dirname):
    for fname in get_figlabels():
        print(fname)
        figure(fname).savefig(dirname + '/' + fname + '.pdf')
